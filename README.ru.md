...
плагин выполняет миграции через Liquibase 4.33.0.
...
### Output file, схемы и образ PostgreSQL
...
| `normalizeOutput` | `true` | Удалять изменчивые строки `pg_dump` |
...
[Gradle Plugin Portal](https://plugins.gradle.org/plugin/io.github.yrashid.actual-schema). Первая
публичная версия недоступна пользовательским проектам до завершения первичного одобрения Portal.
